  ...
  try {
    if (req.method === "GET") {
      ...
      const result = await process.env.DB
        .prepare(
          "SELECT moduleid,modulename,moduledescription,moduletype,module_extra_enable,module_extra_argname,module_extra_column_name,module_extra_func_name,(userid = ?) AS is_self FROM modules WHERE userid=? OR moduletype=1 LIMIT 10000 OFFSET ?"
        )
